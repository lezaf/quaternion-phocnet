'''
@author: georgeretsi
github page: https://github.com/georgeretsi/pytorch-phocnet
'''
import os
...
# from src.io.list_io import LineListIO
from src.string_embeddings.phoc import build_phoc_descriptor, get_unigrams_from_strings
from src.transformations.image_size import check_size
from src.transformations.homography_augmentation import HomographyAugmentation
...
class GWDataset(Dataset):
    ...
    def __init__(self, gw_root_dir, image_extension='.png',
        ...
        for img_filename in img_filenames:
            ...
            with open(os.path.join(gw_root_dir, 'ground_truth', annotation_filename)) as f:
        ...
        '''
        Changed by @lezaf to be compatible with Python3. (BEGIN)
        '''
        ...
        '''
        Changed by @lezaf to be compatible with Python3. (END)
        '''
        ...
        if cv_split_method is not None:
            if cv_split_method == 'almazan':
                ...
                self.split_ids = np.load('./experiments/dataset_loader/almazan_cv_indices.npy') # NOTE: Changed ./dataset_loader/... to ./experiments/dataset_...
            else:
                # fifepages CV
                raise NotImplementedError()
