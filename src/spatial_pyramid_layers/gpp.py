'''
Title:         (part of) pytorch-phocnet
Authors:       georgeretsi
Date:          Dec 19, 2017
Code version:  V1.0
Availability:  https://github.com/georgeretsi/pytorch-phocnet/blob/master/src/cnn_ws/spatial_pyramid_layers/gpp.py
'''
...
'''
TODO-list:
    * Replace hardcoded numbers in 'self.pooling_output_size' calculation in GPP __init__
'''
...
class GPP(nn.Module):
    ...
    def __init__(self, gpp_type='tpp', levels=3, pool_type='max_pool'):
        ...
        if gpp_type == 'spp':
            self.pooling_output_size = sum([4 ** level for level in range(levels)]) * 256
        elif gpp_type == 'tpp':
            self.pooling_output_size = (2 ** levels - 1) * 256
        if gpp_type == 'gpp':
            self.pooling_output_size = sum([h * w for h in levels[0] for w in levels[1]]) * 256
